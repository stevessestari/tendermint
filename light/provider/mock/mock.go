...
type Mock struct {
	...
	evidenceToReport map[string]*types.ConflictingHeadersTrace // hash => evidence
}
...
func New(chainID string, headers map[int64]*types.SignedHeader, vals map[int64]*types.ValidatorSet) *Mock {
	return &Mock{
		...
		headers:          headers,
		vals:             vals,
		evidenceToReport: make(map[string]*types.ConflictingHeadersTrace),
	}
}
...
func (p *Mock) ReportEvidence(ev *types.ConflictingHeadersTrace) error {
	// p.evidenceToReport[string(ev.Hash())] = ev
	return nil
}

func (p *Mock) HasEvidence(ev *types.ConflictingHeadersTrace) bool {
